...
def bleu(stats):
  ''''if len(filter(lambda x: x==0, stats)) > 0:
    return 0'''
  (c, r) = stats[:2]
  #smoothing by adding 1 to x and y
  log_bleu_prec = float(sum([math.log(float(x+1)/(y+1)) for x,y in zip(stats[2::2],stats[3::2])])) / 4.0
  #Smoothing by adding 1 to r
  return math.exp(min([0, 1-float(r+1)/c]) + log_bleu_prec)
